...
void CommonCLI::loadPrefsInt(FILESYSTEM* fs, const char* filename) {
  ...
  if (file) {
    ...
    if (file.available() >= sizeof(_prefs->path_hash_mode)) {
      file.read((uint8_t *)&_prefs->path_hash_mode, sizeof(_prefs->path_hash_mode));
    } else {
      _prefs->path_hash_mode = 0; // default
    }
    if (file.available() >= sizeof(_prefs->autoadd_max_hops)) {
      file.read((uint8_t *)&_prefs->autoadd_max_hops, sizeof(_prefs->autoadd_max_hops));
    } else {
      _prefs->autoadd_max_hops = 0; // default
    }
    if (file.available() >= sizeof(_prefs->default_scope_name)) {
    ...
    if (file.available() >= sizeof(_prefs->default_scope_key)) {
      file.read((uint8_t *)_prefs->default_scope_key, sizeof(_prefs->default_scope_key));
    } else {
      memset(_prefs->default_scope_key, 0, sizeof(_prefs->default_scope_key));
    }
    // tz_offset (from HEAD)
    if (file.available() >= sizeof(_prefs->tz_offset)) {
...
void CommonCLI::savePrefs(FILESYSTEM* fs) {
  ...
  if (file) {
    ...
    file.write((uint8_t *)&_prefs->path_hash_mode, sizeof(_prefs->path_hash_mode));                // new
    file.write((uint8_t *)&_prefs->autoadd_max_hops, sizeof(_prefs->autoadd_max_hops));            // new
    file.write((uint8_t *)_prefs->default_scope_name, sizeof(_prefs->default_scope_name));          // new
    file.write((uint8_t *)_prefs->default_scope_key, sizeof(_prefs->default_scope_key));            // new
    file.write((uint8_t *)&_prefs->tz_offset, sizeof(_prefs->tz_offset));                          // tz_offset
